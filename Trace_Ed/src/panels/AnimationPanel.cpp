...
namespace trace {
	...
	void AnimationPanel::Render(Scene* scene, float deltaTime)
	{
        ...
        if (m_currentClip)
        {
            ...
            case State::PLAY:
            {
                ...
                {
                    AnimationEngine::get_instance()->Animate(m_currentClip, scene, m_elapsed, true, m_clipEnities);
                }
                else if(current_entity && is_sequence_anim)
                {
                    AnimationComponent& anim_comp = current_entity.GetComponent<AnimationComponent>();
                    AnimationEngine::get_instance()->Animate(m_currentClip, scene, m_elapsed, true, anim_comp.entities);
                }
            ...
            case State::PAUSE:
            {
                if (m_lastSelectedFrame != m_currentFrame)
                {
                    ...
                    if (current_entity && is_skeletal_anim)
                    {
                        AnimationEngine::get_instance()->Animate(m_currentClip, scene, t, true, m_clipEnities);
                    }
                    else if (current_entity && is_sequence_anim)
                    {
                        AnimationComponent& anim_comp = current_entity.GetComponent<AnimationComponent>();
                        AnimationEngine::get_instance()->Animate(m_currentClip, scene, t, true, anim_comp.entities);
                    }
    ...
    void AnimationPanel::SetAnimationClip(Scene* scene, Ref<AnimationClip> clip, UUID entity)
    {
        TraceEditor* editor = TraceEditor::get_instance();
        Entity object = scene->GetEntity(entity);
        ...
        if (object && object.HasComponent<AnimationComponent>())
        {
            ...
            {
                anim_comp.InitializeEntities(scene);
        ...
        if (clip->GetType() == AnimationClipType::SKELETAL_ANIMATIOM && object && object.HasComponent<AnimationGraphController>())
        {
            ...
            {
                Entity child = scene->GetChildEntityByName(object, group.first);
                if (child)
                {
    ...
    void AnimationPanel::SetFrameData(Scene* scene, UUID id, AnimationDataType type, void* data, int size)
    {
        if (!m_currentClip)
        {
            return;
