...
namespace trace {
	...
	bool AnimationSequencer::Init()
	{
        ...
        {
            Ref<AnimationClip> clip = ImGuiDragDropResourceCustom<AnimationClip>(rect, id, ANIMATION_CLIP_FILE_EXTENSION);
	...
	void AnimationSequencer::Render(float deltaTime, Scene* scene)
	{
        ...
        if (sequence)
        {
            ...
            {
                m_instance.UpdateElaspedTime(scene, currentTime);
        ...
        if (sequence)
        {
            ...
            for (auto& track : tracks) {
                ...
                if (string_id.value != 0)
                {
                    ...
                    entity = scene->GetEntityByName(string_id);
                }
                ...
                if (ImGui::BeginDragDropTarget())
                {
                    if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("Entity"))
                    {
                        ...
                        if (string_id.value == 0)
                        {
                            ...
                            m_instance.CreateInstance(sequence, scene);
                        }
                    }
                ...
                for (auto& clip : track->GetTrackChannels())
                {
                    ...
                    if (ImGui::BeginPopup("Clip Popup"))
                    {
                        ...
                        case Animation::SequenceTrackType::SKELETAL_ANIMATION_TRACK:
                        {
                            ...
                            {
                                std::string file_path = GetPathFromUUID(chan->GetAnimationClip()->m_assetID).string();
                                TraceEditor::get_instance()->OpenAnimationClip(file_path);
                            }
    ...
    void AnimationSequencer::SetAnimationSequence(Ref<Animation::Sequence> sequence, Scene* scene)
    {
        m_instance.DestroyInstance();
        m_instance.CreateInstance(sequence, scene);
        timelineStart = 0.0f;
