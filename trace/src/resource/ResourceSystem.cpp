...
namespace trace {
	...
	bool ResourceSystem::Init()
	{
		...
		//TODO: Configurable
		result = TextureManager::get_instance()->Init(4096);
		TRC_ASSERT(result, "Failed to initialize texture manager");

		result = FontManager::get_instance()->Init(256);
		TRC_ASSERT(result, "Failed to initialized shader manager");

		result = ShaderManager::get_instance()->Init(4096);
		TRC_ASSERT(result, "Failed to initialized shader manager");

		result = ModelManager::get_instance()->Init(4096);
		TRC_ASSERT(result, "Failed to initialize model manager");

		result = MeshManager::get_instance()->Init(4096);
		TRC_ASSERT(result, "Failed to initialize mesh manager");

		result = PipelineManager::get_instance()->Init(4096);
		TRC_ASSERT(result, "Failed to initialized pipeline manager");

		result = MaterialManager::get_instance()->Init(4096);
	...
	{
		bool result = TextureManager::get_instance()->LoadDefaultTextures();
