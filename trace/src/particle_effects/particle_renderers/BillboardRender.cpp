	...
	void BillBoardRender::Render(ParticleGeneratorInstance* gen_instance, Camera* camera, glm::mat4 transform)
	{
		...
		if (m_velocityAligned)
		{
			...
			while (remaining_particles > 0)
			{
				...
				RenderFunc::SetPipelineTextureData(DefaultAssetsManager::particle_velocity_aligned_pipeline.get(), "u_texture", ShaderResourceStage::RESOURCE_STAGE_INSTANCE, m_texture.get());
				...
				RenderFunc::SetPipelineData(DefaultAssetsManager::particle_velocity_aligned_pipeline.get(), "model", ShaderResourceStage::RESOURCE_STAGE_INSTANCE, &transform, sizeof(glm::mat4));
		...
		else
		{
			...
			while (remaining_particles > 0)
			{
				...
				RenderFunc::SetPipelineData(DefaultAssetsManager::particle_billboard_pipeline.get(), "model", ShaderResourceStage::RESOURCE_STAGE_INSTANCE, &transform, sizeof(glm::mat4));
