...
namespace trace {
	...
	void BloomPass::Init(Renderer* renderer)
	{
		...
		if (AppSettings::is_editor)
		{
...

			{
				...
				clr_bld.render_targets[0].alpha_op = BlendOp::BLEND_OP_ADD;
				clr_bld.render_targets[0].color_op = BlendOp::BLEND_OP_ADD;
				clr_bld.render_targets[0].dst_alpha = BlendFactor::BLEND_ONE;
				clr_bld.render_targets[0].src_alpha = BlendFactor::BLEND_ONE;
				clr_bld.render_targets[0].dst_color = BlendFactor::BLEND_ONE;
				clr_bld.render_targets[0].src_color = BlendFactor::BLEND_ONE;
	...
	void BloomPass::Setup(RenderGraph* render_graph, RGBlackBoard& black_board, int32_t render_graph_index, int32_t draw_index)
	{
	...
	void BloomPass::prefilter(RenderGraph* render_graph, RGBlackBoard& black_board)
	{
		...
		pass->SetRunCB([=](Renderer* renderer, RenderGraph* render_graph, RenderGraphPass* render_graph_pass, int32_t render_graph_index, std::vector<uint32_t>& inputs) {
			...
			RenderFunc::BindPipeline_(m_prefilterPipeline.get(), render_graph_index);
	...
	void BloomPass::downsample(RenderGraph* render_graph, RGBlackBoard& black_board)
	{
		...
		for (uint32_t i = 1; i < bd.samples_count; i++)
		{
			...
			pass->SetRunCB([=](Renderer* renderer, RenderGraph* render_graph, RenderGraphPass* render_graph_pass, int32_t render_graph_index, std::vector<uint32_t>& inputs) {
				...
				RenderFunc::BindPipeline_(m_downSamplePipeline.get(), render_graph_index);
	...
	void BloomPass::upsample(RenderGraph* render_graph, RGBlackBoard& black_board)
	{
		...
		for (int i = bd.samples_count - 2; i >= 0; i--)
		{
			...
			pass->SetRunCB([=](Renderer* renderer, RenderGraph* render_graph, RenderGraphPass* render_graph_pass, int32_t render_graph_index, std::vector<uint32_t>& inputs) {
				...
				RenderFunc::BindPipeline_(m_upSamplePipeline.get(), render_graph_index);
